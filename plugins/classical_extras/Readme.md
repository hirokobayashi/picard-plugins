...
4. "Tags to create" sets the names of the tags that will be created from the sources described above. All these tags will be blanked before filling as specified. Tags specified against more than one source will have later sources appended in the sequence specified, separated by separators as specified.
    ...
    * **Work tags**:
      ...
      - "Tags for top-level (canonical) work". This is the top-level work held in MB. This can be useful for cataloguing and searching (if the library software is capable). Note that this tag is *faithful* - if a movement genuinely belongs to more than one equally-ranked top-level work (e.g. an overture shared between several translations of the same opera), the tag will be multi-valued.
      - "Tags for single-valued work grouping / shuffle key". This is derived from the top-level work but is guaranteed to hold **exactly one** value, chosen deterministically so that every track of the same top work gets the identical string. Use it (rather than the top-level work tag) in library managers that group by exact string equality - e.g. a foobar2000 shuffle pattern such as `%album artist% | %date% | %album% | %work_group%`, which keeps all movements of one work together. The default tag name is work\_group; leave the box blank to suppress the tag.
